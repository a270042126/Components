...
public class AdvertisementView: UIView {
    ...
    public override func layoutSubviews() {
        ...
        adImageView.frame = UIScreen.main.bounds
        gifView?.frame = UIScreen.main.bounds
        
        let skipButtonX = UIScreen.main.bounds.size.width - 90.0
        let skipButtonY: CGFloat = 40
        let skipButtonW: CGFloat = 70
        let skipButtonH: CGFloat = 26
        skipButton.frame = CGRect(x: skipButtonX, y: skipButtonY, width: skipButtonW, height: skipButtonH)
    }
    ...
    private func loadDataSource() {
        ...
        if type == .gif {
            gifView = GifImageOperation(gifData: imageData)
...
extension AdvertisementView {
    ...
    class func saveDataToLocal(_ url: String, data: Data?) -> Bool {
        ...
        let isSuccess = (data! as NSData).write(toFile: filePath, atomically: true)
        return isSuccess
    }
    ...
    class func getAdDataFromLocal(_ url: String) -> Data? {
        ...
        var data: Data? = nil
        do{
            try data = NSData(contentsOfFile: filePath) as Data
        }catch{
            print(error)
        }
        return data
