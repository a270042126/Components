...
public class GifImageOperation: UIView {
    var gifTimer: DispatchSourceTimer?  // GIF播放定时器
    private var isStartAnimation: Bool = false  // 判断是否已经开始了动画 default: false
    ...
    ///   - frame: 显示大小
    ///   - gifData: gif数据
    convenience public init(gifData: Data) {
        ...
        // 设置默认页
        let imageref: CGImage? = CGImageSourceCreateImageAtIndex(gifDataSource, 0, nil)
